...
class ClauseDetector {
    ...
    static void detectClauses(ClausIE clausIE) {
        ...
        for (int i = 0; i < clausIE.clauses.size(); i++) {
            ...
            if (root != null) {
                int index = ancestorOf(clausIE.semanticGraph, root, roots); // recursion needed to deal
    ...
    private static void excludeVertexes(Clause clause) {
        for (int j = 0; j < clause.constituents.size(); j++) {
            if (!(clause.constituents.get(j) instanceof IndexedConstituent)) continue;
     ...
     */
    private static void addNsubjClause(ClausIE clausIE, List<IndexedWord> roots, List<Clause> clauses, IndexedWord subject,
        ...
        for (int i = 0; i < ccs.size(); i++) {
            ...
            if (cop != null) {
                ...
                if (!partmod) {
                    ...
                    clause.constituents.add(new TextConstituent("be " + clauseRoot.word(), Constituent.Type.VERB));
                }
            } else {
                clause.verb = clause.constituents.size();
                if (!partmod) {
            ...
            if (subject.tag().charAt(0) == 'W' && rcmod != null) {
                ...
                if (sub != null)
                    clause.constituents.add(new IndexedConstituent(semanticGraph, sub.getDependent(), Constituent.Type.SUBJECT));
                else
           ...
           //If the clause comes from a partmod construction exclude necesary vertex
            if (partmod) {
                ((IndexedConstituent) clause.constituents.get(clause.subject)).excludedVertexes.add(clauseRoot);
                ...
                if (coppm != null) {
                    ((IndexedConstituent) clause.constituents.get(clause.subject)).excludedVertexes.add(coppm.getDependent());
                    SemanticGraphEdge spm = DpUtils.findFirstOfRelationOrDescendent(outsub, EnglishGrammaticalRelations.SUBJECT);
                    ((IndexedConstituent) clause.constituents.get(clause.subject)).excludedVertexes.add(spm.getDependent());
                }
            }
            ...
            for (SemanticGraphEdge outgoingEdge : outgoingEdges) {
                ...
                //Indirect Object
                    ...
                    //to deal with the possessive relative pronoun     
                        ...
                        rcmod = null;
                    // "regular case"    
                    } else clause.constituents.add(new IndexedConstituent(semanticGraph, dependent, Constituent.Type.IOBJ));
                //Direct Object
                ...
                // are managed within the context of its parent clause)
                    ...
                    for (Clause cl : xcompclauses) {
                        if (xcompsubject != null) {
                            int verb = cl.verb;
                            ((IndexedConstituent) cl.constituents.get(verb)).additionalVertexes.add(xcompsubject);
                        }
                        excludeVertexes(cl);
                    }
                    clause.xcomps.add(clause.constituents.size());
                    clause.constituents.add(new XcompConstituent(semanticGraph, dependent, Constituent.Type.XCOMP, xcompclauses));
            ...
            if (rcmod != null) { //"I saw the house I grew up in", "I saw the house I like", "I saw the man I gave the book" ...
                ...
                if (candidate != null) {
                    ...
                    newSemanticGraph.addEdge(((IndexedConstituent) candidate).getRoot(), rcmod.getGovernor(),
                            EnglishGrammaticalRelations.PREPOSITIONAL_OBJECT, rcmod.getWeight());
    ...
    private static void processRel(SemanticGraphEdge outgoingEdge, SemanticGraph semanticGraph, IndexedWord dependent, SemanticGraphEdge rcmod, Clause clause) {
         ...
         if (pobed != null && pobed.getDependent().tag().charAt(0) == 'W' && rcmod != null) {
             newSemanticGraph.addEdge(dependent, rcmod.getGovernor(), EnglishGrammaticalRelations.PREPOSITIONAL_OBJECT, pobed.getWeight());
             ...
             newSemanticGraph.addEdge(posspobj.getGovernor(), rcmod.getGovernor(), EnglishGrammaticalRelations.POSSESSION_MODIFIER,
                     posspobj.getWeight());
    ...
    private static Constituent searchCandidateAdverbial(Clause clause) {
        for (Constituent c : clause.constituents) {
            ...
            if (root.tag().equals("IN") && !((IndexedConstituent) c).getSemanticGraph().hasChildren(root))
                return c;
        }
        return null;
    }
    ...
    */
    private static Constituent createPossConstituent(SemanticGraph semanticGraph, SemanticGraphEdge poss, SemanticGraphEdge rcmod,
        ...
        newSemanticGraph.addEdge(poss.getGovernor(), rcmod.getGovernor(), EnglishGrammaticalRelations.POSSESSION_MODIFIER, weight);
    ...
    */
    ...
    private static void addApposClause(ClausIE clausIE, IndexedWord subject, IndexedWord object) {
        ...
        clause.verb = 1;
        clause.complement = 2;
        clause.constituents.add(new IndexedConstituent(clausIE.semanticGraph, subject, Constituent.Type.SUBJECT));
        clause.constituents.add(new TextConstituent(clausIE.options.appositionVerb, Constituent.Type.VERB));
        clause.constituents.add(new IndexedConstituent(clausIE.semanticGraph, object, Constituent.Type.COMPLEMENT));
    ...
    private static void addPossessiveClause(ClausIE clausIE, IndexedWord subject, IndexedWord object) {
        ...
        if (rcmod != null) {
            ...
            clause.constituents.add(new IndexedConstituent(newSemanticGraph, subject, Collections.<IndexedWord> emptySet(), excludesub,
                    Type.SUBJECT));
        }
        clause.constituents.add(new TextConstituent(clausIE.options.possessiveVerb, Constituent.Type.VERB));
        clause.constituents.add(new IndexedConstituent(newSemanticGraph, object, Collections.<IndexedWord> emptySet(), excludeobj,
    ...
    private static void excludeVertexPoss(List<SemanticGraphEdge> outedobj, Set<IndexedWord> excludeobj, ClausIE clausIE) {
        for (SemanticGraphEdge ed : outedobj) {
            if (DpUtils.isAdvcl(ed)
                    || DpUtils.isAdvmod(ed)
                    || DpUtils.isAnyObj(ed) // currently everything is excluded except prep and infmod
     ...
     * @param clausIE clausIE
     * @param subject The subject of the clause
     * @param verb The verb of the clause
    ...
    private static void addParataxisClause(ClausIE clausIE, IndexedWord root, IndexedWord parroot, List<IndexedWord> roots) {
        ...
        if (subject != null) {
            Constituent subjectConst = new IndexedConstituent(clausIE.semanticGraph, subject.getDependent(), Type.SUBJECT);
