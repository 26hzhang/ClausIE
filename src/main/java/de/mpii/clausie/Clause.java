...
public class Clause {
	/* -- type definition -- */
	/** Clause types */
	public enum Type { SV, SVC, SVA, SVO, SVOO, SVOC, SVOA, EXISTENTIAL, UNKNOWN }
	/* -- member variables -- */
	...
	int verb = -1;
	// They are lists because some times the parsers (probably an error) generates more than one constituent of each
	// type, e.g., more than one dobj produced by parser for "The man who I told the fact is dead."
	...
	public boolean hasCandidateAdverbial() {
		if (adverbials.isEmpty()) return false;
		if (relativeAdverbial) return true;
		// is there an adverbial that occurs after the verb?
		if (((IndexedConstituent) constituents.get(adverbials.get(adverbials.size() - 1))).getRoot().index() >
	 ...
	 */
	int noComplements() { return dobjects.size() + (complement < 0 ? 0 : 1) + xcomps.size() + ccomps.size(); }
	...
	public String toString(Options options) {
		...
		for (int index = 0; index < constituents.size(); index++) {
			...
			case ACOMP: s.append("ACOMP"); break;
			case ADVERBIAL:
				s.append("A");
				if (options != null) {
					switch (getFlag(index, options)) {
					case IGNORE: s.append("-"); break;
					case OPTIONAL: s.append("?"); break;
					case REQUIRED: s.append("!"); break;
					}
				}
				break;
			case CCOMP: s.append("CCOMP"); break;
			case COMPLEMENT: s.append("C"); break;
			case DOBJ: s.append("O"); break;
			case IOBJ: s.append("IO"); break;
			case SUBJECT: s.append("S"); break;
			case UNKOWN: s.append("?"); break;
			case VERB: s.append("V"); break;
			case XCOMP: s.append("XCOMP"); break;
			}
			s.append(": ");
			if (!(constituent instanceof IndexedConstituent)) s.append("\"");
		...
		for (int i : adverbials) {
			...
			if (constituents.get(verb) instanceof IndexedConstituent &&
				...
				if (i == index) {
					...
					return !(Type.SVA.equals(type) || Type.SVOA.equals(type)) ? Flag.OPTIONAL : Flag.REQUIRED;
				}
				first = false;
			}
		}
